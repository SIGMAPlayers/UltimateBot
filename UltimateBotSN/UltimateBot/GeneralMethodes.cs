...
namespace MyBot
{
    public static class GeneralMethods
    {
        ...
        {
            for (int i = 0; i < GameSettings.AtkList.Count; i++)
            {
                foreach (Pirate p in GameSettings.game.GetMyLivingPirates())
                {
                    if (GameSettings.AtkList[i].Pirate.Id == p.Id)
                        GameSettings.AtkList[i].Pirate = p;

                }
            }

            for (int i = 0; i < GameSettings.allPirates.DefenderList.Count; i++)
            {
                foreach (Pirate p in GameSettings.game.GetMyLivingPirates())
                {
                    if (GameSettings.allPirates.DefenderList[i].Pirate.Id == p.Id)
                        GameSettings.allPirates.DefenderList[i].Pirate = p;
